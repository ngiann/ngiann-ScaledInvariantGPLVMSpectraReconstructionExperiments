...
let
    X,rec,res,net,fmin, = gplvm(Yobs, S, iterations = 200_000, Q = 3, H = 30, seed = 1);
    JLD2.save("gplvm_scaled_coil_2D.jld2", "X", X, "rec", rec, "res", res, "net", net, "fmin", fmin)
end

# run scale-invariance GPLVM and save results
let
    X,rec,res,net,fmin,c,cvar = scaleinvariantgplvm(Yobs,S,iterations = 200_000, Q=3, H = 30, seed=1);
